...
```
很多时候，我们希望通过一种方式，让同一个自定义组件，展现出不一样的内容。vue也提供了一个叫插槽的东西**slot**,
```
<!DOCTYPE html>
<html>
<head>
...
```
如上可以通过<slot>标签将组件中间的内容插入到自定义组件模板里面。
...
```
通过这种方式，可以将一个组件套其他组件或者套很多组件。灵活又简洁。
